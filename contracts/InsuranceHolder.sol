...
contract InsuranceHolder is HelperFunctions {
    ...
    function RegisterHolder(uint256 intialcover, address id) public returns(string message) {
    ...
    function Archive(address id) public returns(string message) {
        if (id == address(0)) {
            message = "Invalid address";
            emit GeneralLogger(message);
            return message;
    ...
    function PayCover(address holderId, uint256 amount) public payable returns(string message) {
