...
contract GovernmentLedger is HelperFunctions {
    ...
    function RegisterPerson(string name, string sname, string lat, string long, address id) public returns(string message) {
        if (id == address(0)) {
    ...
    function UpdatePersonName(string name, address id) public returns(string message) {
        if (id == address(0)) {
            message = "Invalid address";
            emit GeneralLogger(message);
            return message;
    ...
    //@Dev updates the specified Persons details using the address
    function UpdatePersonSurname(string sname, address id) public returns(string message) {
        if (id == address(0)) {
            message = "Invalid address";
            emit GeneralLogger(message);
            return message;
    ...
    function CitizenExists(address id) view public returns(bool exists) {
        require(id != address(0));
        return People[id].Activated && People[id].Alive ? true : false;
    }

    //@Dev Gets the specfied Person using the given
    function GetPersonInternal(address id) view public returns(string name, string sname, address Id) {
        if (id == address(0)) {
    ...
    function GetPerson(address id) view public returns(bytes32 name, bytes32 sname, address Id) {
        if (id == address(0)) {
            emit GeneralLogger("Invalid address");
            return ("Invalid address", "", id);
        }
